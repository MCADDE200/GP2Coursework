...
char* AudioClass::loadWAVFile(const char* fn, int& chan, int& samplerate, int& bps, int& size)
{
	char buffer[4];
...
unsigned int AudioClass::loadSoundFile(const char* filename)
{
	bool found = false;
	...
	datas.push_back(dataStruct(sourceID, (!found ? bufferID : -1), soundData, filename));
	return sourceID;
}

void AudioClass::deleteSoundFile(unsigned int id)
{}

void AudioClass::playSoundFile(unsigned int id)
{
	alSourcePlay(id);
}
void AudioClass::playSoundFile(unsigned int id, glm::vec3& pos)
{
